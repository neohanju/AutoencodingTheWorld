...
# =============================================================================
# Autoencoder for 50x50
# =============================================================================
class AE_51x51(nn.Module):
    def __init__(self, num_in_channels, num_filters=36):
        super().__init__()

        # encoder layers
        # (batch_size) x NC x 51 x 51
        self.encoder = nn.Sequential(
            nn.Conv2d(num_in_channels, num_filters, 5, 2),
            nn.BatchNorm2d(num_filters),
            nn.ReLU(True),
            # (batch_size) x NF x 24 x 24
            nn.Conv2d(num_filters, 2 * num_filters, 5, 2),
            nn.BatchNorm2d(2 * num_filters),
            nn.ReLU(True),
            # (batch_size) x 2NF x 10 x 10
            nn.Conv2d(num_filters, 4 * num_filters, 5, 2),
            nn.BatchNorm2d(4 * num_filters),
            nn.ReLU(True)
            # (batch_size) x 4NF x 3 x 3
        )
        self.decoder = nn.Sequential(
...
# Autoencoder [original]
# =============================================================================
class AE_BN(nn.Module):  # autoencoder struction from "Learning temporal regularity in video sequences"
    def __init__(self, num_in_channels, num_filters=512, z_size=128):
