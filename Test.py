...
import numpy as np
import Datasets.RGBImageSet_masked_loss as dset
...
parser.add_argument('--dataroot', default='/media/leejeyeol/74B8D3C8B8D38750/Data/CVC-ClinicDB/train', help='path to dataset')
parser.add_argument('--net', default='/home/leejeyeol/Git/AutoencodingTheWorld/Reconstructed_Image/network_epoch_990_fold_0.pth', help="path of networks.(to continue training)")
...
# todo fold number
fold_number = 0
dataloader = torch.utils.data.DataLoader(dset.RGBImageSet_masked_loss(options.dataroot,type='train', centered=False, fold_number=fold_number),batch_size=options.batchSize, shuffle=True, num_workers=options.workers)
...
for epoch in range(options.iteration):
    for i, (data, mask_, data_name) in enumerate(dataloader, 0):
        ...
        np.save(os.path.join(error_save_path, data_name), error_image)
