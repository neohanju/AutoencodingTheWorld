...
test_evaluate('/home/leejeyeol/git/VAE_4_DET_AB/training_result/AE_LTR_20170421-175250_avenue-enter-exit-leejeyeol-H97-HD3');
    

function [] = test_evaluate(cost_base_path_)
    ...
    savefile=fopen(fullfile(cost_base_path, '../evaluation.csv'),'a')
    fprintf(savefile,'%s,,',model)
	...
	%         minima_indices = [persistent_features(:,1); globalMinIndex];
	    ...
	    fprintf('----------------------------------------\n')
        fprintf(savefile,',')
        fprintf(savefile,'%f,%f,%f,%f,%f',tp,fp,fn,tp/(tp+fp),tp/(tp+fn))
